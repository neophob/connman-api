'use strict';
/*
 * Javascript interface around Connman manager
 * See: https://git.kernel.org/cgit/network/connman/connman.git/tree/doc/manager-api.txt
 *
 */
...
const debug = require('debug')('connman');

const dbus = new DBus();

const DEFAULT_TIMEOUT = 10000;

const ConnMan = module.exports = function(enableAgent) {
  var self = this;
  ...
  self.manager = null;
  self.technologies = {};
  self.enableAgent = enableAgent || false;
};
...
ConnMan.prototype.getService = function(serviceName, callback) {
  ...
  self.getServices(function(err, services) {
    var properties = services[serviceName];
    var service = null;
    if (!properties) {
    ...
    switch(properties.Type) {
      case 'ethernet':
        service = new Wired(self);
        service.init(serviceName, function(err) {
            if(err) return callback(err);
            callback(null, service,properties);
        });
        break;
      case 'wifi':
        service = new Wifi(self);
        service.init(serviceName, function(err) {
            if(err) return callback(err);
            callback(null, service,properties);
        });
      ...
      // ToDo: add bluetooth
      default:
        callback(new Error("No suitable interface found for '"+properties.Type+"'"));
        break;
    }
  });
};
  ...
  self.getServices(type,function(err,services) {
    ...
    for(var key in services) {
      if(matches(services[key],query)) {
        ...
        //debug("found service '"+serviceData.Name+"'");
        break;
      }
    }
    if (!serviceName) {
